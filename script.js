...
function onChange() {
  ...
  information = information.then((movieInfo) => {
      axios
        .get(`https://api.themoviedb.org/3/movie/${movieInfo.data.results.at(0).id}`, {
          params: {
            api_key: "0dcabfe51b80fa2de3e80d7d256e0e81",
        ...
        .then((movieData) => {
          ...
          movieTrailer.src = `https://www.youtube.com/embed/${
            trailers.at(0).key
          }`;
          ...
          description.innerHTML = `-- ${movieInfo.data.results.at(0).release_date} -- ${movieInfo.data.results.at(0).original_language} -- ${movieInfo.data.results.at(0).popularity}`;
